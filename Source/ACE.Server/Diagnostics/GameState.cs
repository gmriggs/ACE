...
using ProtoBuf.Meta;
using ACE.Entity;
using ACE.Server.Diagnostics.Serialization;
using ACE.Server.Managers;
using ACE.Server.WorldObjects;

namespace ACE.Server.Diagnostics.Entity
{
    [ProtoContract]
    public class GameState
    {
        ...
        public void MapFields()
        {
            ...
            foreach (var landblock in LandblockManager.ActiveLandblocks)
            {
                foreach (var worldObject in landblock.worldObjects.Values)
                {
                    if (!Callbacks.Sendable(worldObject))
                        continue;

                    WorldObjects.Add(worldObject.Guid, worldObject);
                }
            }
            //Console.WriteLine("GameState count=" + WorldObjects.Count);
        }
