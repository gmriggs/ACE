...
namespace ACE.Server.WorldObjects
{
    ...
    public class SpellProjectile : WorldObject
    {
        ...
        {
            Console.WriteLine("Spellproject.OnCollideEnvironment()");
            ProjectileImpact();
        }

        public override void OnCollideObject(WorldObject _target)
        {
            Console.WriteLine("Spellproject.OnCollideObject(" + targetGuid.Full.ToString("X8") + ")");
            SpellTable spellTable = DatManager.PortalDat.SpellTable;
            SpellBase spell = spellTable.Spells[spellId];
            ...
            {
                Console.WriteLine("Hit " + target.Guid.Full.ToString("X8") + "(" + target.Name + ") instead");
                OnCollideEnvironment();
                return;
            }
